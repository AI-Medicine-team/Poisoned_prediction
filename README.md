# Explainable Artificial Intelligence for Predicting Adverse Outcomes in Acute Poisoning: Development and External Validation

# Overview

This repository contains scripts that implement an explainable artificial intelligence (AI) framework for predicting adverse outcomes—specifically, incomplete recovery or death—in patients with acute poisoning, while performing temporal external validation and providing transparent, clinician-oriented explanations.

Notebook	Description
DNN, LSTM	Data preprocessing and cleaning
Development and training of DNN and LSTM models
Prediction and outcome evaluation
ML 	Data preprocessing and cleaning
Development and training of ML models
Prediction and outcome evaluation
prediction_plot	Evaluates the final model's predictive performance, Interprets predictions via SHAP analysis, Structured SHAP outputs for an LLM to generate patient-specific risk assessments, Clinical explanations, enhancing interpretability
valid_data_predict	Temporal External Validation of the Model

